...
use super::{CollectionValuesDump, MAX_KEYS, validate_key, validate_value};

#[derive(Error, Debug)]
pub enum CollectionValuesWriterError {
...
impl CollectionValuesWriter {
    ...
    pub fn try_load(data_dir: PathBuf) -> Result<Self, CollectionValuesWriterError> {
    ...
    /// Sets a collection value. Validates key and value, checks max keys limit.
    /// Silently overwrites if the key already exists.
    pub fn set(&mut self, key: String, value: String) -> Result<(), CollectionValuesWriterError> {
        debug!(
            "Setting collection value: key='{}', value_size={}",
            key,
            value.len()
        );

        validate_key(&key).context("Invalid collection value key")?;
        validate_value(&value).context("Invalid collection value")?;
    ...
    /// Persists collection values to disk. Only writes if there are uncommitted changes.
    pub fn commit(&mut self, data_dir: PathBuf) -> Result<(), CollectionValuesWriterError> {
...
mod tests {
    ...
    fn test_empty_writer() {
        let writer = CollectionValuesWriter::empty();
    ...
    #[test]
    fn test_set_and_get() {
        let mut writer = CollectionValuesWriter::empty();
    ...
    fn test_commit_and_reload() {
        ...
        // Reload from disk
        let writer = CollectionValuesWriter::try_load(base_dir).expect("Failed to load");
